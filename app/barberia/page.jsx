...
// ============================================
// 👻 COMPONENTE ABANDONADA (Citas no completadas)
// ============================================
...
export default function BarberiaDashboard() {
  ...
  const cargarProspectos = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/barberia/prospectos`);
      const data = await res.json();
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [user, cargarConfig, cargarStats, cargarCitas, cargarAbandonadas, cargarProspectos]);
  ...
  const notificarProspecto = async (id) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/barberia/prospectos/${id}/notificar`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }
      });
      const data = await res.json();
      ...
      <div className="container mx-auto px-4 py-6 max-w-4xl">
        ...
        <div className="flex gap-1 mb-4 overflow-x-auto">
          {[
            ...
            { key: 'prospectos', icon: '👤', badge: prospectosPendientes.length },
            { key: 'reportes', icon: '📊' },
            { key: 'config', icon: '⚙️' },
          ].map((tab) => (
        ...
        <button onClick={() => { cargarStats(); cargarCitas(); cargarAbandonadas(); cargarProspectos(); setCountdown(10); }}

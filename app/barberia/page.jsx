  ...
  return (
    <div className="bg-gradient-to-br from-yellow-400 to-amber-500 rounded-2xl p-4 text-white shadow-lg">
      ...
      <div className="bg-white/20 rounded-xl p-3 mb-3">
        ...
        <div className="flex items-center gap-2 text-sm">💰 Total gastado: ${cliente.total_gastado?.toLocaleString('es-CL')}</div>
      </div>
...
function ReportesPanel({ plan }) {
  ...
  const descargarExcel = async () => {
    ...
    setExportando('excel');
    try {
      window.open(`${API_BASE}/api/barberia/export/excel?periodo=${periodo}&plan=${plan}`, '_blank');
  ...
  const descargarPDF = async (tipo = 'basico') => {
    ...
    setExportando('pdf');
    try {
      window.open(`${API_BASE}/api/barberia/export/pdf?periodo=${periodo}&tipo=${tipo}&plan=${plan}`, '_blank');
...
export default function BarberiaDashboard() {
  ...
  const cargarNoAsistieron = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/barberia/no-asistieron`);
      const data = await res.json();
  ...
  const cargarSinAgendar = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/barberia/sin-agendar`);
      const data = await res.json();
  ...
  const cargarVips = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/barberia/vips`);
      const data = await res.json();
  ...
  const cargarRecordatorios = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/barberia/recordatorios`);
      const data = await res.json();
  ...
  const notificarNoAsistio = async (id) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/barberia/no-asistieron/${id}/notificar`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }
      });
      const data = await res.json();
  ...
  const notificarSinAgendar = async (id) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/barberia/sin-agendar/${id}/notificar`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }
      });
      const data = await res.json();
  ...
  const enviarPromoVip = async (id) => {
    try {
      const res = await fetch(`${API_BASE}/api/barberia/vips/${id}/promo`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }
      });
      const data = await res.json();
  ...
  const enviarRecordatorio = async (id) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/barberia/recordatorios/${id}/enviar`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }
      });
      const data = await res.json();
  ...
  const rescatarTodosNoAsistieron = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/barberia/no-asistieron/rescatar-todos`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }
      });
      const data = await res.json();
  ...
  const enviarTodosRecordatorios = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/barberia/recordatorios/enviar-todos`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }
      });
      const data = await res.json();
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600">
      ...
      <div className="container mx-auto px-4 py-6 max-w-4xl">
        ...
        <div className="flex gap-1 mb-4 overflow-x-auto pb-2">
          {[
            { key: 'citas', icon: '💈', label: '', badge: citas.filter(c => ['Confirmada', 'En Proceso'].includes(c.estado)).length },
            { key: 'noAsistio', icon: '😞', label: '', badge: noAsistieronPendientes.length },
            { key: 'sinAgendar', icon: '📋', label: '', badge: sinAgendarPendientes.length },
            { key: 'vips', icon: '⭐', label: '', badge: vips.length },
            { key: 'recordatorios', icon: '⏰', label: '', badge: recordatoriosPendientes.length },

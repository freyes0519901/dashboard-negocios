...
function ReportesPanel({ plan }) {
  ...
  const descargarExcel = async () => {
    ...
    setExportando('excel');
    try {
      window.open(`${API_BASE}/api/carrito/export/excel?periodo=${periodo}&plan=${plan}`, '_blank');
  ...
  const descargarPDF = async (tipo = 'basico') => {
    ...
    setExportando('pdf');
    try {
      window.open(`${API_BASE}/api/carrito/export/pdf?periodo=${periodo}&tipo=${tipo}&plan=${plan}`, '_blank');
